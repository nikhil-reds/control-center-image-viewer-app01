...
export default function CrtControlPage() {
  const [isSending, setIsSending] = useState(false);

  // Activate CRT module on mount
  useEffect(() => {
    async function activateModule() {
      try {
        await fetch("/api/pdf-control", {
          ...
          body: JSON.stringify({ action: "activate", pdfId: "pdf-1" }),
        });
      } catch {
